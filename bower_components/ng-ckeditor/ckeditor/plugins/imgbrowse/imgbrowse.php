...
class imgbrowse {
  ...
  private static function isThumbnailFile($name) {
  ...
  public function getMenuImgs() {
    ...
    foreach($obdr as $fileobj) {
      ...
      if($fileobj->isFile() && in_array($fileobj->getExtension(), $this->imgext) && preg_match('/\.thumb\.[^\s]+?$/',$name)){
        $re['imgs'] .= '<span><img src="'. $site . $this->root . $this->imgdr . $name .'" alt="'. $name .'" height="50" /></span>';
      }
